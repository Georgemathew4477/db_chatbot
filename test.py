...
def rewrite_query_with_groq(query):
    prompt = f"""
        You are a medical assistant. Rephrase the user's question using medical terminology that 
        better aligns with clinical documents, without changing the meaning. Do not answer the question.
...
def ask_bot(question, embedder, index, chunks, sources, preferences, k=3):
    # 🔄 Step 1: Rewrite query using Groq LLM
    def rewrite_query_with_groq(query):
        prompt = f"""
You are a medical assistant. Rephrase the user's question using formal clinical terminology to improve matching with official healthcare guidelines. Do not answer it—just rewrite it.
...
Rewritten:"""
    ...
    rewritten_query = rewrite_query_with_groq(question)

    # 🧠 Step 2: Retrieve using the rewritten query
    q_embed = embedder.encode([rewritten_query])[0]
    ...
    # 🧬 Glucose injection if available
    ...
    # 🧾 Final context with patient data
    context += f"\n\n{glucose_line}{user_info}"

    # 🤖 Prompt for Groq answer
...
Be honest. If unsure, say so and recommend a doctor."""
    ...
    # Final answer uses the original query (not the rewritten one)
